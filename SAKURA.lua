...
DevSakura:set(ServerSAKURA.."IdSAKURA",SUDO) 
else 
print('\27[1;31m┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\nلم يتم حفظ ايدي المطور الاساسي ارسله مره اخرى\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉') 
end 
os.execute('lua SAKURA.lua') 
end 
if not DevSakura:get(ServerSAKURA.."TokenSAKURA") then 
...
DevSakura:set(ServerSAKURA.."TokenSAKURA",token) 
end  
else 
...
DevSakura:set(DevSakura:get(ServerSAKURA.."TokenSAKURA"):match("(%d+)")..'Sakura:Error',true)
...
config = {
SUDO = DevSakura:get(ServerSAKURA.."IdSAKURA"),
token = DevSakura:get(ServerSAKURA.."TokenSAKURA"),
bot_id = DevSakura:get(ServerSAKURA.."TokenSAKURA"):match("(%d+)"),
sudo_users = {DevSakura:get(ServerSAKURA.."IdSAKURA")},
}
create(config, "./config.lua")   
file = io.open("RUN.sh", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/SAKURA
token="]]..DevSakura:get(ServerSAKURA.."TokenSAKURA")..[["
...
./tg -s ./SAKURA.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("TOFY", "w")  
...
local Load_SAKURA = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevSakura:del(ServerSAKURA.."IdSAKURA");DevSakura:del(ServerSAKURA.."TokenSAKURA")
end 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == SAKURA then  
var = true  
end  
return var  
end 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:SudoBot:',msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:ManagerAll:',msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:AdminAll:',msg.sender_user_id_) 
...
------ VIP MEMBER ALL ------
function VipAll(msg) 
local Status = DevSakura:sismember(SAKURA..'Sakura:VipAll:',msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:AbsConstructor:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or Sudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
local Status = DevSakura:sismember(SAKURA..'Sakura:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevSakura:sismember(SAKURA..'Sakura:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevSakura:sismember(SAKURA..'Sakura:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevSakura:sismember(SAKURA..'Sakura:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
...
if DevSakura:sismember(SAKURA..'Sakura:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevSakura:sismember(SAKURA..'Sakura:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevSakura:sismember(SAKURA..'Sakura:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevSakura:sismember(SAKURA..'Sakura:MuteAll:', user_id) then
var = true
else
var = false
end
...
if plugin.SAKURA and msg then
FilesText = plugin.SAKURA(msg)
end
end
end
...
DevSakura:sadd(SAKURA..'Sakura:AbsConstructor:'..IdGps,IdAbsConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevSakura:sadd(SAKURA..'Sakura:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevSakura:sadd(SAKURA..'Sakura:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevSakura:sadd(SAKURA..'Sakura:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevSakura:sadd(SAKURA..'Sakura:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادمنيه')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevSakura:sadd(SAKURA..'Sakura:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevSakura:set(SAKURA.."Sakura:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevSakura:set(SAKURA.."Sakura:Groups:Welcomes"..IdGps,v.Welcomes)   
...
var = true  
elseif tonumber(user_id) == tonumber(SAKURA) then  
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:SudoBot:', user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:ManagerAll:', user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:AdminAll:', user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:VipAll:', user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:AbsConstructor:'..chat_id, user_id) then
var = true
elseif DevSakura:sismember(SAKURA..'Sakura:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevSakura:sismember(SAKURA..'Sakura:Constructor:'..chat_id, user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:Managers:'..chat_id, user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:Admins:'..chat_id, user_id) then
var = true  
elseif DevSakura:sismember(SAKURA..'Sakura:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local abs = (SAKURA..'Sakura:Filters:'..msg.chat_id_)
if abs then
local names = DevSakura:hkeys(abs)
...
function absmoned(chat_id, user_id, msg_id, text, offset, length) local tt = DevSakura:get(SAKURA..'endmsg') or '' tdcli_function ({ ID = "SendMessage", chat_id_ = chat_id, reply_to_message_id_ = msg_id, disable_notification_ = 0, from_background_ = 1, reply_markup_ = nil, input_message_content_ = { ID = "InputMessageText", text_ = text..'\n\n'..tt, disable_web_page_preview_ = 1, clear_draft_ = 0, entities_ = {[0]={ ID="MessageEntityMentionName", offset_=offset, length_=length, user_id_=user_id }, }, }, }, dl_cb, nil) end
function ChCheck(msg)
local var = true 
if DevSakura:get(SAKURA.."DevAbs2") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevSakura:get(SAKURA..'DevAbs2')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevSakura:get(SAKURA..'Sakura:textch:user') then
...
if DataText == '/delyes' and DevSakura:get(SAKURA..'yes'..data.sender_user_id_) == 'delyes' then
...
if DevSakura:sismember(SAKURA..'Sakura:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevSakura:sismember(SAKURA..'Sakura:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevSakura:sismember(SAKURA..'Sakura:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevSakura:sismember(SAKURA..'Sakura:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevSakura:sismember(SAKURA..'Sakura:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevSakura:sismember(SAKURA..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايه • ' else donky = '' end
if DevSakura:sismember(SAKURA..'Sakura:Constructor:'..data.chat_id_, data.sender_user_id_) or DevSakura:sismember(SAKURA..'Sakura:Managers:'..data.chat_id_, data.sender_user_id_) or DevSakura:sismember(SAKURA..'Sakura:Admins:'..data.chat_id_, data.sender_user_id_) or DevSakura:sismember(SAKURA..'Sakura:VipMem:'..data.chat_id_, data.sender_user_id_) or DevSakura:sismember(SAKURA..'Sakura:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevSakura:sismember(SAKURA..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevSakura:srem(SAKURA..'Sakura:Constructor:'..data.chat_id_,data.sender_user_id_)
DevSakura:srem(SAKURA..'Sakura:Managers:'..data.chat_id_,data.sender_user_id_)
DevSakura:srem(SAKURA..'Sakura:Admins:'..data.chat_id_,data.sender_user_id_)
...
DevSakura:incrby(SAKURA..'Sakura:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
...
if text and DevSakura:get(SAKURA.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevSakura:get(SAKURA.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
...
if text and text:match('^'..(DevSakura:get(SAKURA..'Sakura:NameBot') or "ساكورا")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevSakura:get(SAKURA..'Sakura:NameBot') or "ساكورا")..' ','')
end
if data.message_.content_.text_ then
local NewCmmd = DevSakura:get(SAKURA.."Set:Cmd:Group:New1"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
...
if text and DevSakura:get(SAKURA.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
...
if DevSakura:sismember(SAKURA.."Coomds"..msg.chat_id_,DEV_ABBAS) then
...
if DevSakura:sismember(SAKURA.."Coomds"..msg.chat_id_,text1[2]) then
...
DevSakura:sadd(SAKURA.."Sakura:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
if text == '/start' then  
if not DevSakura:get(SAKURA..'Sakura:Start:Time'..msg.sender_user_id_) then
tdcli_function({ID="GetUser",user_id_=DevId},function(arg,dp) 
local inline = {{{text="⧉ ︙ المطور ︙ ⧉",url="t.me/"..(dp.username_ or "SAKURAV15")}}}
...
DevSakura:setex(SAKURA..'Sakura:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
...
DevSakura:incr(SAKURA..'Sakura:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevSakura:set(SAKURA.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevSakura:get(SAKURA.."Sakura:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(SAKURA) then 
...
DevSakura:del(SAKURA..'Sakura:AbsConstructor:'..msg.chat_id_)
...
DevSakura:sadd(SAKURA..'Sakura:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevSakura:srem(SAKURA..'Sakura:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevSakura:sadd(SAKURA..'Sakura:Admins:'..msg.chat_id_, admins[i].user_id_)
...
DevSakura:sadd(SAKURA.."Sakura:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevSakura:sadd(SAKURA.."Sakura:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevSakura:srem(SAKURA.."Sakura:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevSakura:srem(SAKURA.."Sakura:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
...
DevSakura:set(SAKURA..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
local idf = tostring(msg.chat_id_)
if not DevSakura:sismember(SAKURA.."Sakura:Groups",msg.chat_id_) and not idf:match("^(%d+)") and not SudoBot(msg) then
...
if not Manager(msg) then 
local filter = DevSakura:smembers(SAKURA.."Sakura:FilterPhoto"..msg.chat_id_)
...
if not Manager(msg) then 
local filter = DevSakura:smembers(SAKURA.."Sakura:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Manager(msg) then 
local filter = DevSakura:smembers(SAKURA.."Sakura:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
DevSakura:del(SAKURA..'Sakura:Audio:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:del(SAKURA..'Sakura:Photo:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:del(SAKURA..'Sakura:Stecker:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:del(SAKURA..'Sakura:Video:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:del(SAKURA..'Sakura:File:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:del(SAKURA..'Sakura:Text:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
DevSakura:srem(SAKURA..'Sakura:Manager:GpRed'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
if msg.content_.text_ then
...
DevSakura:del(SAKURA.."Sakura:Audio:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:Photo:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:Stecker:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:Video:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:File:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:Text:AllRed"..msg.content_.text_)
DevSakura:del(SAKURA.."Sakura:Sudo:AllRed",msg.content_.text_)
return false
end
end
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevSakura:get(SAKURA..'Sakura:Add:GpRed'..msg.sender_user_id_..''..msg.chat_id_..'')
...
DevSakura:del(SAKURA..'Sakura:Add:GpRed'..msg.sender_user_id_..''..msg.chat_id_..'')
local SaveGpRed = DevSakura:get(SAKURA..'Sakura:Add:GpText'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then DevSakura:set(SAKURA..'Sakura:Video:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevSakura:set(SAKURA..'Sakura:File:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevSakura:set(SAKURA..'Sakura:Stecker:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevSakura:set(SAKURA..'Sakura:Voice:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevSakura:set(SAKURA..'Sakura:Audio:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.audio_.audio_.persistent_id_) 
end
...
DevSakura:set(SAKURA..'Sakura:Photo:GpRed'..SaveGpRed..''..msg.chat_id_..'', photo_in_group) 
end
if msg.content_.animation_ then DevSakura:set(SAKURA..'Sakura:Gif:GpRed'..SaveGpRed..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
...
if msg.content_.text_ and not DevSakura:get(SAKURA..'Sakura:Lock:GpRed'..msg.chat_id_) then 
if DevSakura:get(SAKURA..'Sakura:Video:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA..'Sakura:Video:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end 
if DevSakura:get(SAKURA..'Sakura:File:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA..'Sakura:File:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end 
if DevSakura:get(SAKURA..'Sakura:Voice:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA..'Sakura:Voice:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end
if DevSakura:get(SAKURA..'Sakura:Audio:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA..'Sakura:Audio:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end
if DevSakura:get(SAKURA..'Sakura:Photo:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA..'Sakura:Photo:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end
if DevSakura:get(SAKURA..'Sakura:Gif:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA..'Sakura:Gif:GpRed'..msg.content_.text_..''..msg.chat_id_..'')) 
end 
if DevSakura:get(SAKURA..'Sakura:Stecker:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA..'Sakura:Stecker:GpRed'..msg.content_.text_..''..msg.chat_id_..''))
end
if DevSakura:get(SAKURA..'Sakura:Text:GpRed'..msg.content_.text_..''..msg.chat_id_..'') then
function SAKURATEAM(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevSakura:get(SAKURA..'Sakura:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevSakura:get(SAKURA..'Sakura:Text:GpRed'..msg.content_.text_..''..msg.chat_id_..'')
...
getUser(msg.sender_user_id_, SAKURATEAM)
end
end
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevSakura:get(SAKURA.."Sakura:Add:AllRed"..msg.sender_user_id_)
...
DevSakura:del(SAKURA.."Sakura:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevSakura:get(SAKURA.."Sakura:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevSakura:set(SAKURA.."Sakura:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevSakura:set(SAKURA.."Sakura:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevSakura:set(SAKURA.."Sakura:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevSakura:set(SAKURA.."Sakura:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevSakura:set(SAKURA.."Sakura:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevSakura:set(SAKURA.."Sakura:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevSakura:set(SAKURA.."Sakura:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
...
if msg.content_.text_ and not DevSakura:get(SAKURA..'Sakura:Lock:AllRed'..msg.chat_id_) then
if DevSakura:get(SAKURA.."Sakura:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA.."Sakura:Video:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA.."Sakura:File:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA.."Sakura:Voice:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA.."Sakura:Audio:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA.."Sakura:Photo:AllRed"..msg.content_.text_))
end
if  DevSakura:get(SAKURA.."Sakura:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevSakura:get(SAKURA.."Sakura:Gif:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevSakura:get(SAKURA.."Sakura:Stecker:AllRed"..msg.content_.text_))
end
if DevSakura:get(SAKURA.."Sakura:Text:AllRed"..msg.content_.text_) then
function SAKURATEAM(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevSakura:get(SAKURA..'Sakura:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevSakura:get(SAKURA.."Sakura:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
--  end functions SAKURA --
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= SAKURA then
floods = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevSakura:get(SAKURA.."Sakura:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevSakura:setex(SAKURA.."Sakura:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
----- START MSG CHECKS -----
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(SAKURA) then
DevSakura:set(SAKURA..'Sakura:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevSakura:get(SAKURA..'Sakura:PinnedMsg'..msg.chat_id_)
if pin_id and DevSakura:get(SAKURA..'Sakura:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevSakura:get(SAKURA..'Sakura:viewget'..msg.sender_user_id_) then
...
if DevSakura:get(SAKURA..'Sakura:Groups:Welcomes'..msg.chat_id_) then
text = DevSakura:get(SAKURA..'Sakura:Groups:Welcomes'..msg.chat_id_)
else
text = '• نورت حبي \n• [firstname lastname] \n• [@username]'
end
local text = text:gsub('firstname',(result.first_name_ or ''))
local text = text:gsub('lastname',(result.last_name_ or ''))
local text = text:gsub('username',(result.username_ or 'SAKURAV15'))
Dev_Abs(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
if DevSakura:get(SAKURA.."Sakura:Lock:Welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end
--      New User Add      --
elseif msg_type == 'MSG:NewUserAdd' then
if not DevSakura:get(SAKURA..'Sakura:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = SAKURA,offset_ = 0,limit_ = 1},function(extra,abbas,success) 
...
local abs = 'Sakura:Spam:Text'..msg.chat_id_
if not DevSakura:get(SAKURA..abs) then
sens = 400
else
sens = tonumber(DevSakura:get(SAKURA..abs))
end
if DevSakura:get(SAKURA..'Sakura:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("سکس") or text:match("سکسی") or text:match("کسی") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") then
if DevSakura:get(SAKURA.."Sakura:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
...
NameBot = (DevSakura:get(SAKURA..'Sakura:NameBot') or 'ساكورا')
local SAKURATEAM = {"لتكول بوت اسمي "..NameBot.." 😒🔪","اسمي القميل "..NameBot.." 😚♥️","عندي اسم تره 😒💔","صيحولي "..NameBot.." كافي بوت 😒🔪","انت البوت لك 😒💔"} 
DevAbs2 = math.random(#SAKURATEAM) 
Dev_Abs(msg.chat_id_, msg.id_, 1, SAKURATEAM[DevAbs2] , 1, 'html') 
return false
end
if text == 'اسم البوت' or text == 'البوت شنو اسمه' or text == 'شسمه البوت' or text == 'البوت شسمه' then
NameBot = (DevSakura:get(SAKURA..'Sakura:NameBot') or 'ساكورا') 
local SAKURATEAM = {"اسمي القميل "..NameBot.." 😚♥️","هلاا يروحيي وياكك "..NameBot.." 😻♥️"} 
DevAbs2 = math.random(#SAKURATEAM) 
Dev_Abs(msg.chat_id_, msg.id_, 1, SAKURATEAM[DevAbs2] , 1, 'html') 
return false
end
if text and text == (DevSakura:get(SAKURA..'Sakura:NameBot') or 'ساكورا') then 
NameBot = (DevSakura:get(SAKURA..'Sakura:NameBot') or 'ساكورا')
...
DevSakura:set(SAKURA..'Sakura:GameNum'..msg.chat_id_,name)
...
if DevSakura:get(SAKURA..'Sakura:Lock:Games'..msg.chat_id_) then
Num = math.random(1,6)
DevSakura:set(SAKURA.."GAMES"..msg.chat_id_,Num) 
TEST = [[
➀     ➁     ➂     ➃     ➄     ➅
↓     ↓     ↓     ↓     ↓     ↓
👊 ‹› 👊🏻 ‹› 👊🏼 ‹› 👊🏽 ‹› 👊🏾 ‹› 👊🏿
...
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum2'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum3'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
...
DevAbs2 = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','📥','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧚‍♂️','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕓','🕤','⌛️','📅','👩‍⚖️','👨‍🎨'};
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum4'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
name = string.gsub(name,'😸','😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠️','💀💀💀☠️💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼')
...
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum5'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
...
if text == DevSakura:get(SAKURA..'Sakura:GameNum5'..msg.chat_id_) then
if not DevSakura:get(SAKURA..'Sakura:Games:Ids'..msg.chat_id_) then 
DevSakura:incrby(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
...
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum6'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
...
if text == DevSakura:get(SAKURA..'Sakura:GameNum6'..msg.chat_id_) then
if not DevSakura:get(SAKURA..'Sakura:Games:Ids'..msg.chat_id_) then 
DevSakura:incrby(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
...
name = DevAbs2[math.random(#DevAbs2)]
DevSakura:set(SAKURA..'Sakura:GameNum7'..msg.chat_id_,name)
DevSakura:del(SAKURA..'Sakura:Games:Ids'..msg.chat_id_)
...
if text == DevSakura:get(SAKURA..'Sakura:GameNum7'..msg.chat_id_) then
if not DevSakura:get(SAKURA..'Sakura:Games:Ids'..msg.chat_id_) then 
DevSakura:incrby(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
...
if text == DevSakura:get(SAKURA..'Sakura:GameNum8'..msg.chat_id_) then
if not DevSakura:get(SAKURA..'Sakura:Games:Ids'..msg.chat_id_) then 
DevSakura:incrby(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
...
if DevSakura:get(SAKURA.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
local CountAdd = DevSakura:get(SAKURA.."Sakura:NumRolet"..msg.chat_id_..msg.sender_user_id_)
local CountAll = DevSakura:scard(SAKURA..'Sakura:ListRolet'..msg.chat_id_)
...
DevSakura:sadd(SAKURA..'Sakura:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevSakura:sadd(SAKURA..'Sakura:AbsConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
local msguser = tonumber(DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevSakura:get(SAKURA..'Sakura:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
local listpv = DevSakura:smembers(SAKURA.."Sakura:Users")
...
⧉️↫ 𝐒𝐀𝐊𝐔𝐑𝐀 𝐓𝐄𝐀𝐌
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⧉️↫[𝐜𝐡𝐚𝐧𝐧𝐞𝐥 𝐬𝐚𝐤𝐮𝐫𝐚](http://t.me/SAKURAV15) 

⧉️↫[𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑](http://t.me/MMLMMM)

⧉️↫[𝐓𝐨 𝐭𝐚𝐥𝐤 𝐭𝐨 𝐮𝐬](http://t.me/SAKURATEM_BOT)
]]
Dev_Abs(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
i = i + 1
if DevSakura:get(SAKURA..'Save:UserName'..v.user_id_) then
text = text..""..i.."~ : [@"..DevSakura:get(SAKURA..'Save:UserName'..v.user_id_).."]\n"
else
...
if DevSakura:sismember(SAKURA.."Sakura:AbsConstructor:"..msg.chat_id_,result.id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local SAKURATEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,SAKURATEAM, 1, 'md') 
...
if DevSakura:sismember(SAKURA.."Sakura:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local SAKURATEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,SAKURATEAM, 1, 'md') 
local SAKURATEAM = {"لكك جرجف احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش فاشل لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","دمشي لك ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪","حبيبي راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..SAKURATEAM[math.random(#SAKURATEAM)]..'', 1, 'md') 
end 
...
local SAKURATEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,SAKURATEAM, 1, 'md') 
local SAKURATEAM = {"مواححح افيش عافيههه😍🔥💗","امممووااهحح شهلعسل🥺🍯💘","مواححح،ءوفف اذوب🤤💗"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..SAKURATEAM[math.random(#SAKURATEAM)]..'', 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local SAKURATEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,SAKURATEAM, 1, 'md') 
local SAKURATEAM = {"تتعال ححب محتاجيك🙂🍭","تعال يولل استاذكك ايريدككك😒🔪","يمعوود تعاال يريدوكك🤕♥️","تعال لكك ديصيحوك😐🖤"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..SAKURATEAM[math.random(#SAKURATEAM)]..'', 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
if DevSakura:sismember(SAKURA..'Sakura:ManagerAll:',result.sender_user_id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevSakura:sismember(SAKURA..'Sakura:AdminAll:',result.sender_user_id_) then
adminall = 'الادمنيه العامين • ' else adminall = '' end
if DevSakura:sismember(SAKURA..'Sakura:VipAll:',result.sender_user_id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevSakura:sismember(SAKURA..'Sakura:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevSakura:sismember(SAKURA..'Sakura:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevSakura:sismember(SAKURA..'Sakura:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevSakura:sismember(SAKURA..'Sakura:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevSakura:sismember(SAKURA..'Sakura:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevSakura:sismember(SAKURA..'Sakura:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevSakura:srem(SAKURA..'Sakura:ManagerAll:', result.sender_user_id_)
DevSakura:srem(SAKURA..'Sakura:AdminAll:', result.sender_user_id_)
DevSakura:srem(SAKURA..'Sakura:VipAll:', result.sender_user_id_)
...
sudobot = 'المطورين • ' else sudobot = '' end
if DevSakura:sismember(SAKURA..'Sakura:ManagerAll:',result.id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevSakura:sismember(SAKURA..'Sakura:AdminAll:',result.id_) then
adminall = 'الادمنيه العامين • ' else adminall = '' end
if DevSakura:sismember(SAKURA..'Sakura:VipAll:',result.id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevSakura:sismember(SAKURA..'Sakura:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevSakura:sismember(SAKURA..'Sakura:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevSakura:sismember(SAKURA..'Sakura:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevSakura:sismember(SAKURA..'Sakura:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevSakura:sismember(SAKURA..'Sakura:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevSakura:sismember(SAKURA..'Sakura:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
DevSakura:srem(SAKURA..'Sakura:ManagerAll:', result.id_)
DevSakura:srem(SAKURA..'Sakura:AdminAll:', result.id_)
DevSakura:srem(SAKURA..'Sakura:VipAll:', result.id_)
...
DevSakura:sadd(SAKURA..'Sakura:ManagerAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevSakura:sadd(SAKURA..'Sakura:ManagerAll:',result.id_)
...
DevSakura:sadd(SAKURA..'Sakura:ManagerAll:',user)
...
DevSakura:srem(SAKURA..'Sakura:ManagerAll:',user)
...
function raf_reply(extra, result, success)
DevSakura:sadd(SAKURA..'Sakura:AdminAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevSakura:sadd(SAKURA..'Sakura:AdminAll:',result.id_)
...
DevSakura:sadd(SAKURA..'Sakura:AdminAll:',user)
...
DevSakura:srem(SAKURA..'Sakura:AdminAll:',user)
...
function raf_reply(extra, result, success)
DevSakura:sadd(SAKURA..'Sakura:VipAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevSakura:sadd(SAKURA..'Sakura:VipAll:',result.id_)
...
DevSakura:sadd(SAKURA..'Sakura:VipAll:',user)
...
DevSakura:srem(SAKURA..'Sakura:VipAll:',user)
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..SAKURA)
local GetInfo = JSON.decode(Check)
...
if DevSakura:get(SAKURA..'Sakura:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ then if msg.reply_to_message_id_ ~= 0 then DevSakura:sadd(SAKURA.."Sakura:cleaner"..msg.chat_id_, msg.id_) else DevSakura:sadd(SAKURA.."Sakura:cleaner"..msg.chat_id_, msg.id_) end end end
...
local list = DevSakura:smembers(SAKURA..'Sakura:ManagerAll:')
...
local list = DevSakura:smembers(SAKURA..'Sakura:VipAll:')
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevSakura:get(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevSakura:sismember(SAKURA..'Sakura:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevSakura:sismember(SAKURA..'Sakura:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevSakura:sismember(SAKURA..'Sakura:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevSakura:sismember(SAKURA..'Sakura:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevSakura:sismember(SAKURA..'Sakura:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevSakura:get(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevSakura:sismember(SAKURA..'Sakura:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevSakura:sismember(SAKURA..'Sakura:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevSakura:sismember(SAKURA..'Sakura:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevSakura:sismember(SAKURA..'Sakura:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevSakura:sismember(SAKURA..'Sakura:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
local user_nkt = tonumber(DevSakura:get(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevSakura:sismember(SAKURA..'Sakura:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevSakura:sismember(SAKURA..'Sakura:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevSakura:sismember(SAKURA..'Sakura:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevSakura:sismember(SAKURA..'Sakura:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevSakura:sismember(SAKURA..'Sakura:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevSakura:sismember(SAKURA..'Sakura:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevSakura:sismember(SAKURA..'Sakura:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
...
if result.id_ then
if DevSakura:sismember(SAKURA..'Sakura:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevSakura:sismember(SAKURA..'Sakura:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
...
function SAKURATEAM(extra,abbas,success)
if abbas.username_ then username = '@'..abbas.username_ else username = 'لا يوجد' end
local function getpro(extra, abbas, success) 
local msgsday = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..SAKURA..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevSakura:get(SAKURA..'Sakura:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevSakura:get(SAKURA..'Sakura:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevSakura:get(SAKURA..'Sakura:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevSakura:get(SAKURA..'Sakura:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
newpicid = DevSakura:get(SAKURA.."Sakura:AllIds:Text")
...
local new_id = DevSakura:get(SAKURA.."Sakura:GpIds:Text"..msg.chat_id_)
...
if DevSakura:get(SAKURA.."Sakura:AllIds:Text") then
newallid = DevSakura:get(SAKURA.."Sakura:AllIds:Text")
...
if DevSakura:get(SAKURA.."Sakura:AllIds:Text") then
notpicid = DevSakura:get(SAKURA.."Sakura:AllIds:Text")
...
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevSakura:get(SAKURA.."Sakura:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevSakura:set(SAKURA.."Sakura:Groups:Links"..msg.chat_id_,LinkGroup) 
...
local wel = DevSakura:get(SAKURA..'Sakura:Groups:Welcomes'..msg.chat_id_)
if wel then
...
local pv = DevSakura:smembers(SAKURA.."Sakura:Users")
local sendok = 0
for i = 1, #pv do
...
DevSakura:srem(SAKURA.."Sakura:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevSakura:smembers(SAKURA.."Sakura:Groups")
local w = 0
local q = 0
for i = 1, #group do
...
DevSakura:srem(SAKURA.."Sakura:Groups",group[i]) 
changeChatMemberStatus(group[i], SAKURA, "Left")
w = w + 1
end
...
if Cleaner(msg) then
if DevSakura:get(SAKURA..'Sakura:Lock:Clean'..msg.chat_id_) then 
...
DevSakura:del(SAKURA.."Sakura:Admins:Rd"..msg.chat_id_)
DevSakura:del(SAKURA.."Sakura:Managers:Rd"..msg.chat_id_)
DevSakura:del(SAKURA.."Sakura:Constructor:Rd"..msg.chat_id_)
DevSakura:del(SAKURA.."Sakura:BasicConstructor:Rd"..msg.chat_id_)
DevSakura:del(SAKURA.."Sakura:SudoBot:Rd"..msg.chat_id_)
end
end
...
if tonumber(admins[i].user_id_) ~= tonumber(SAKURA) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SAKURATEAM, 14, string.len(msg.sender_user_id_))
DevSakura:del(SAKURA..'Sakura:ManagerAll:')
end
end
if AbsConstructor(msg) then
...
local Managers = DevSakura:smembers(SAKURA..'Sakura:Managers:'..msg.chat_id_)
local admins = DevSakura:smembers(SAKURA..'Sakura:Admins:'..msg.chat_id_)
local vipmem = DevSakura:smembers(SAKURA..'Sakura:VipMem:'..msg.chat_id_)
local donky = DevSakura:smembers(SAKURA..'User:Donky:'..msg.chat_id_)
...
if not DevSakura:get(SAKURA..'Sakura:Spam:Text'..msg.chat_id_) then
spam_c = 400
else
spam_c = DevSakura:get(SAKURA..'Sakura:Spam:Text'..msg.chat_id_)
end
if DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقييد"     
elseif DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevSakura:hget(SAKURA.."Sakura:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
if DevSakura:get(SAKURA.."Sakura:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevSakura:get(SAKURA.."Sakura:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقييد"   
elseif DevSakura:get(SAKURA.."Sakura:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحه"    
end
if DevSakura:get(SAKURA..'Sakura:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه'end
if DevSakura:get(SAKURA..'Sakura:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفله' else mute_photo = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفله' else mute_video = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفله' else mute_gifs = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Music'..msg.chat_id_) then mute_music = 'مقفله' else mute_music = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفله' else mute_in = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفله' else mute_voice = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفله' else mute_edit = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفله' else lock_pin = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفله' else lock_sticker = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفله' else lock_tgservice = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفله' else lock_wp = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفله' else lock_htag = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفله' else lock_tag = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Location'..msg.chat_id_) then lock_location = 'مقفله' else lock_location = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفله' else lock_contact = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:English'..msg.chat_id_) then lock_english = 'مقفله' else lock_english = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفله' else lock_arabic = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفله' else lock_forward = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Document'..msg.chat_id_) then lock_file = 'مقفله' else lock_file = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفله' else markdown = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفله' else lock_spam = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل' else lock_Join = 'مفتوح' end
if DevSakura:get(SAKURA.."Sakura:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفله' else send_welcome = 'مفتوحه' end
if DevSakura:get(SAKURA..'Sakura:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end
if DevSakura:get(SAKURA..'Sakura:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end
...
local rules = DevSakura:get(SAKURA..'Sakura:rules'..msg.chat_id_)
...
if not DevSakura:get(SAKURA..'Sakura:Brg:Abs'..msg.chat_id_) then
...
if not DevSakura:get(SAKURA..'Sakura:Age:Abs'..msg.chat_id_) then
...
if not DevSakura:get(SAKURA..'Sakura:Mean:Abs'..msg.chat_id_) then
...
if msg.content_.photo_ then
if DevSakura:get(SAKURA..'Sakura:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
local ManagerList = DevSakura:scard(SAKURA.."Sakura:Managers:"..ChatId) or 0
local MutedList = DevSakura:scard(SAKURA.."Sakura:Muted:"..ChatId) or 0
local TkeedList = DevSakura:scard(SAKURA.."Sakura:Sakura:Tkeed:"..ChatId) or 0
...
DevSakura:set(SAKURA..'Sakura:SudosGp'..msg.sender_user_id_..msg.chat_id_,"abs")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
sendDocument(DevId, 0, 0, 1, nil, './SAKURA.lua', dl_cb, nil)
...
list = DevSakura:smembers(SAKURA..'Sakura:Users') 
if msg.content_.text_ then
for k,v in pairs(list) do 
AbsText = "الرساله"
...
list = DevSakura:smembers(SAKURA..'Sakura:Groups') 
if msg.content_.text_ then
for k,v in pairs(list) do 
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
local list = DevSakura:smembers(SAKURA..'Sakura:Users')   
for k,v in pairs(list) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⧉︙تم اذاعة رسالتك بالتوجيه \n⧉︙‏الى ↫ ❨ "..#list.." ❩ مشترك \n ✓", 1, 'md')
...
DevSakura:setex(SAKURA.."Sakura:Send:Gp:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
local SAKURATEAM = '⧉︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⧉︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n⧉︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SAKURATEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevSakura:get(SAKURA.."Sakura:Send:Gp:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⧉︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevSakura:del(SAKURA.."Sakura:Send:Gp:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false
end 
local list = DevSakura:smembers(SAKURA.."Sakura:Groups") 
if msg.content_.text_ then
...
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
AbsText = "الصوره"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(list) do 
AbsText = "المتحركه"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(list) do 
AbsText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(list) do 
AbsText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(list) do 
AbsText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(list) do 
AbsText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(list) do 
AbsText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevSakura:set(SAKURA..'Sakura:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
DevSakura:set(SAKURA..'DelManagerRep'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
text = msg.content_.text_
...
DevSakura:set(SAKURA.."DelSudoRep",msg.content_.text_)
return false 
end end
...
dp = 'متحركه 🎭'
elseif DevSakura:get(SAKURA.."Sakura:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevSakura:get(SAKURA.."Sakura:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevSakura:get(SAKURA.."Sakura:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevSakura:get(SAKURA.."Sakura:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صوره 🎇'
elseif DevSakura:get(SAKURA.."Sakura:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevSakura:get(SAKURA.."Sakura:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevSakura:get(SAKURA.."Sakura:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
DevSakura:del(SAKURA..'Sakura:Voice:GpRed'..v..msg.chat_id_)
DevSakura:del(SAKURA..'Sakura:Audio:GpRed'..v..msg.chat_id_)
DevSakura:del(SAKURA..'Sakura:Photo:GpRed'..v..msg.chat_id_)
DevSakura:del(SAKURA..'Sakura:Stecker:GpRed'..v..msg.chat_id_)
DevSakura:del(SAKURA..'Sakura:Video:GpRed'..v..msg.chat_id_)
DevSakura:del(SAKURA..'Sakura:File:GpRed'..v..msg.chat_id_)
...
dp = 'متحركه 🎭'
elseif DevSakura:get(SAKURA.."Sakura:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevSakura:get(SAKURA.."Sakura:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevSakura:get(SAKURA.."Sakura:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevSakura:get(SAKURA.."Sakura:Photo:AllRed"..v) then
dp = 'صوره 🎇'
elseif DevSakura:get(SAKURA.."Sakura:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevSakura:get(SAKURA.."Sakura:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevSakura:get(SAKURA.."Sakura:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..''..k..'~ (`'..v..'`) ↫ {*'..dp..'*}\n' 
end
...
DevSakura:del(SAKURA.."Sakura:Gif:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:Voice:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:Audio:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:Photo:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:Stecker:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:Video:AllRed"..v)
DevSakura:del(SAKURA.."Sakura:File:AllRed"..v)
...
HelpList ={'Sakura:Help','Sakura:Help1','Sakura:Help2','Sakura:Help3','Sakura:Help4','Sakura:Help5','Sakura:Help6'}
...
local Users = DevSakura:smembers(SAKURA.."bot:userss")
local Groups = DevSakura:smembers(SAKURA..'bot:groups')
local Sudos = DevSakura:smembers(SAKURA.."Sakura:SudoBot:")
if DevSakura:get('ABS_PROX:'..SAKURA..'name_bot') then
DevSakura:set(SAKURA..'Sakura:NameBot',(DevSakura:get('ABS_PROX:'..SAKURA..'name_bot') or 'ساكورا'))
end
for i = 1, #Users do
local id = Users[i]
if id:match("^(%d+)") then
...
if DevSakura:get(SAKURA.."bot:group:link"..Groups[i]) then
DevSakura:set(SAKURA.."Sakura:Groups:Links"..Groups[i],DevSakura:get(SAKURA.."bot:group:link"..Groups[i]))
end
if DevSakura:get(SAKURA.."welcome:"..Groups[i]) then
DevSakura:set(SAKURA..'Sakura:Groups:Welcomes'..Groups[i],DevSakura:get(SAKURA.."welcome:"..Groups[i]))
end
local list1 = DevSakura:smembers(SAKURA..'Sakura:absmonsh:'..Groups[i])
for k,v in pairs(list1) do
DevSakura:sadd(SAKURA.."Sakura:AbsConstructor:"..Groups[i], v)
end
local list2 = DevSakura:smembers(SAKURA..'Sakura:monsh:'..Groups[i])
for k,v in pairs(list2) do
DevSakura:sadd(SAKURA.."Sakura:Constructor:"..Groups[i], v)
end
local list3 = DevSakura:smembers(SAKURA..'Sakura:monshid:'..Groups[i])
for k,v in pairs(list3) do
DevSakura:sadd(SAKURA.."Sakura:BasicConstructor:"..Groups[i], v)
end
local list4 = DevSakura:smembers(SAKURA..'Sakura:owners:'..Groups[i])
for k,v in pairs(list4) do
DevSakura:sadd(SAKURA.."Sakura:Managers:"..Groups[i], v)
end
local list5 = DevSakura:smembers(SAKURA..'Sakura:admins:'..Groups[i])
for k,v in pairs(list5) do
DevSakura:sadd(SAKURA.."Sakura:Admins:"..Groups[i], v)
end
local list6 = DevSakura:smembers(SAKURA..'Sakura:vipmem:'..Groups[i])
for k,v in pairs(list6) do
DevSakura:sadd(SAKURA.."Sakura:VipMem:"..Groups[i], v)
end
local list7 = DevSakura:smembers(SAKURA..'Sakura:Cleaner:'..Groups[i])
for k,v in pairs(list7) do
DevSakura:sadd(SAKURA.."Sakura:Cleaner:"..Groups[i], v)
end
...
DevSakura:del('ABS_PROX:') DevSakura:del(SAKURA..'bot:') DevSakura:del(SAKURA..'ABS_PROX:')
DevSakura:del(SAKURA..'user:msgs'..msg.chat_id_) DevSakura:del(SAKURA..'Sakura:')
end
if text == 'تصحيح الاخطاء' then
if not DevSakura:get(SAKURA..'Sakura:Error') then
local ErrorGroups = DevSakura:smembers(SAKURA..'Sakura:Groups')
for i = 1, #ErrorGroups do
local ErrorList1 = DevSakura:smembers(SAKURA..'Sakura:Constructor:'..ErrorGroups[i])
for k,v in pairs(ErrorList1) do
DevSakura:sadd(SAKURA.."Sakura:BasicConstructorError:"..ErrorGroups[i], v)
end
local ErrorList2 = DevSakura:smembers(SAKURA..'Sakura:BasicConstructor:'..ErrorGroups[i])
for k,v in pairs(ErrorList2) do
DevSakura:sadd(SAKURA.."Sakura:ConstructorError:"..ErrorGroups[i], v)
end
DevSakura:del(SAKURA.."Sakura:Constructor:"..ErrorGroups[i]) DevSakura:del(SAKURA.."Sakura:BasicConstructor:"..ErrorGroups[i])
local ErrorList22 = DevSakura:smembers(SAKURA..'Sakura:BasicConstructorError:'..ErrorGroups[i])
for k,v in pairs(ErrorList22) do
DevSakura:sadd(SAKURA.."Sakura:BasicConstructor:"..ErrorGroups[i], v)
end
local ErrorList11 = DevSakura:smembers(SAKURA..'Sakura:ConstructorError:'..ErrorGroups[i])
for k,v in pairs(ErrorList11) do
DevSakura:sadd(SAKURA.."Sakura:Constructor:"..ErrorGroups[i], v)
end
DevSakura:del(SAKURA.."Sakura:ConstructorError:"..ErrorGroups[i]) DevSakura:del(SAKURA.."Sakura:BasicConstructorError:"..ErrorGroups[i])
end
io.popen("mkdir Files")
DevSakura:set(SAKURA..'Sakura:Error',true)
...
UsedPrc=`free -m | awk 'NR==2{printf "%sMB/%sMB {%.2f%}\n", $3,$2,$3*100/$2 }'`
...
DevSakura:incr(SAKURA..'Sakura:EditMsg'..result.chat_id_..result.sender_user_id_)
...
local GetMsgPin = DevSakura:get(SAKURA..'Sakura:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevSakura:del(SAKURA..'Sakura:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevSakura:del(SAKURA..'Sakura:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
local PvList = DevSakura:smembers(SAKURA..'Sakura:Users')  
for k,v in pairs(PvList) do 
tdcli_function({ID='GetChat',chat_id_ = v},function(arg,data) end,nil) 
end 
local GpList = DevSakura:smembers(SAKURA..'Sakura:Groups') 
for k,v in pairs(GpList) do 
tdcli_function({ID='GetChat',chat_id_ = v},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=SAKURA,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevSakura:srem(SAKURA..'Sakura:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevSakura:srem(SAKURA..'Sakura:Groups',v)  
end
...
DevSakura:sadd(SAKURA..'Sakura:Groups',v)  
end end,nil) end
end
end 
